...
extern "C" {
    ...
    pub fn eglChooseConfig(dpy: *mut c_void, attrib_list: *const c_int, configs: *mut c_void, config_size: c_int, num_config: *mut c_int) -> c_int;
    pub fn eglCreateWindowSurface(dpy: *mut c_void, config: *mut c_void, win: *mut c_void, attrib_list: *const c_int) -> *mut c_void;
    pub fn eglCreateContext(dpy: *mut c_void, config: *mut c_void, share_context: *mut c_void, attrib_list: *const c_int) -> *mut c_void;
    pub fn eglMakeCurrent(dpy: *mut c_void, draw: *mut c_void, read: *mut c_void, ctx: *mut c_void) -> c_int;
...
pub mod egl_mock {
    ...
    pub unsafe fn eglGetDisplay(_: *mut c_void) -> *mut c_void { 0x1 as *mut c_void }
    pub unsafe fn eglInitialize(_: *mut c_void, maj: *mut c_int, min: *mut c_int) -> c_int { *maj = 1; *min = 4; 1 }
    pub unsafe fn eglChooseConfig(_: *mut c_void, _: *const c_int, cfg: *mut c_void, _: c_int, n: *mut c_int) -> c_int { *n = 1; *(cfg as *mut usize) = 0xABC; 1 }
    pub unsafe fn eglCreateWindowSurface(_: *mut c_void, _: *mut c_void, _: *mut c_void, _: *const c_int) -> *mut c_void { NEXT.fetch_add(1, Ordering::Relaxed) as *mut c_void }
    pub unsafe fn eglCreateContext(_: *mut c_void, _: *mut c_void, _: *mut c_void, _: *const c_int) -> *mut c_void { NEXT.fetch_add(1, Ordering::Relaxed) as *mut c_void }
    pub unsafe fn eglMakeCurrent(_: *mut c_void, _: *mut c_void, _: *mut c_void, _: *mut c_void) -> c_int { 1 }
    pub unsafe fn eglSwapBuffers(_: *mut c_void, _: *mut c_void) -> c_int { 1 }
    pub unsafe fn eglDestroyContext(_: *mut c_void, _: *mut c_void) -> c_int { 1 }
    pub unsafe fn eglDestroySurface(_: *mut c_void, _: *mut c_void) -> c_int { 1 }
    pub unsafe fn eglGetError() -> c_int { 0x3000 }
}
#[cfg(any(not(target_os = "android"), feature = "mock", test))]
use egl_mock::*;
...
extern "C" {
    ...
    pub fn glShaderSource(shader: c_int, count: c_int, string: *const *const c_char, length: *const c_int);
    pub fn glCompileShader(shader: c_int);
    pub fn glGetShaderiv(shader: c_int, pname: c_int, params: *mut c_int);
    pub fn glGetShaderInfoLog(shader: c_int, bufSize: c_int, length: *mut c_int, infoLog: *mut c_char);
    ...
    pub fn glGetProgramInfoLog(program: c_int, bufSize: c_int, length: *mut c_int, infoLog: *mut c_char);
    ...
    pub fn glTexImage2D(target: c_int, level: c_int, internalformat: c_int, width: c_int, height: c_int, border: c_int, format: c_int, ty: c_int, pixels: *const c_void);
    ...
    pub fn glVertexAttribPointer(idx: c_int, size: c_int, ty: c_int, normalized: c_int, stride: c_int, ptr: *const c_void);
...
pub mod gl_mock {
    ...
    pub unsafe fn glGenBuffers(n: c_int, buffers: *mut c_int) {
        for i in 0..n { *buffers.offset(i as isize) = NEXT.fetch_add(1, Ordering::Relaxed); }
    }
    ...
    pub unsafe fn glGenTextures(n: c_int, textures: *mut c_int) {
        for i in 0..n { *textures.offset(i as isize) = NEXT.fetch_add(1, Ordering::Relaxed); }
    }
    ...
    pub unsafe fn glVertexAttribPointer(_: c_int, _: c_int, _: c_int, _: c_int, _: c_int, _: *const c_void) {}
...
impl EglDisplay {
    pub fn get_default() -> Result<Self, i32> {
        ...
        if h.is_null() { return Err(0x3008); }
        Ok(Self { handle: h })
    }
    pub fn initialize(&mut self) -> Result<(i32, i32), i32> {
        let mut maj: c_int = 0; let mut min: c_int = 0;
        if unsafe { eglInitialize(self.handle, &mut maj, &mut min) } == 1 {
            Ok((maj, min))
        } else { Err(unsafe { eglGetError() }) }
    }
    pub fn choose_config(&mut self, attribs: &[c_int]) -> Result<*mut c_void, i32> {
        let mut cfg: *mut c_void = core::ptr::null_mut();
        let mut n: c_int = 0;   
        if unsafe { eglChooseConfig(self.handle, attribs.as_ptr(), &mut cfg as *mut *mut c_void as *mut c_void, 1, &mut n) } == 1 && n > 0 {
        Ok(cfg) 
        } else { Err(unsafe { eglGetError() }) }
    }
    pub fn create_window_surface(&mut self, config: *mut c_void, win: &k1_sys::NativeWindow) -> Result<*mut c_void, i32> {
        let surf = unsafe { eglCreateWindowSurface(self.handle, config, win.as_raw() as *mut c_void, core::ptr::null()) };
        if surf.is_null() { Err(unsafe { eglGetError() }) } else { Ok(surf) }
    }
    pub fn create_context(&mut self, config: *mut c_void, version: i32) -> Result<*mut c_void, i32> {
        let attribs = [EGL_CONTEXT_CLIENT_VERSION, version, EGL_NONE];
        let ctx = unsafe { eglCreateContext(self.handle, config, EGL_NO_CONTEXT, attribs.as_ptr()) };
        if ctx.is_null() { Err(unsafe { eglGetError() }) } else { Ok(ctx) }
    }
    pub fn make_current(&self, surf: *mut c_void, ctx: *mut c_void) -> Result<(), i32> {
        if unsafe { eglMakeCurrent(self.handle, surf, surf, ctx) } == 1 { Ok(()) } else { Err(unsafe { eglGetError() }) }
    }
    pub fn swap_buffers(&self, surf: *mut c_void) -> Result<(), i32> {
        if unsafe { eglSwapBuffers(self.handle, surf) } == 1 { Ok(()) } else { Err(unsafe { eglGetError() }) }
    }
...
impl Shader {
    pub fn from_source(ty: c_int, source: &str) -> Result<Self, i32> {
        ...
        if handle == 0 { return Err(0x9999); }
        let src_ptr = source.as_bytes().as_ptr() as *const c_char;
        let len = source.len() as c_int;
        unsafe { glShaderSource(handle, 1, &src_ptr, &len); }
        unsafe { glCompileShader(handle); }
        let mut status: c_int = 0;
        unsafe { glGetShaderiv(handle, GL_COMPILE_STATUS, &mut status); }
        if status == 0 {
            unsafe { glDeleteShader(handle); }
            return Err(0x9998);
        }
        Ok(Self { handle, shader_type: ty })
    }
...
impl Program {
    pub fn new() -> Result<Self, i32> {
        ...
        if h == 0 { Err(0x9997) } else { Ok(Self { handle: h }) }
    }
    pub fn attach_shader(&self, shader: &Shader) {
        unsafe { glAttachShader(self.handle, shader.handle()); }
    }
    pub fn link(&self) -> Result<(), i32> {
        unsafe { glLinkProgram(self.handle); }
        let mut status: c_int = 0;
        unsafe { glGetProgramiv(self.handle, GL_LINK_STATUS, &mut status); }
        if status == 0 { Err(0x9996) } else { Ok(()) }
    }
...
impl Buffer {
    pub fn new(target: c_int) -> Result<Self, i32> {
        let mut h: c_int = 0;
        unsafe { glGenBuffers(1, &mut h); }
        if h == 0 { Err(0x9995) } else { Ok(Self { handle: h, target }) }
    }
    pub fn bind(&self) { unsafe { glBindBuffer(self.target, self.handle); } }
    pub fn upload<T>(&self, data: &[T], usage: c_int) {
        self.bind();
        unsafe { glBufferData(self.target, (data.len() * core::mem::size_of::<T>()) as isize, data.as_ptr() as *const c_void, usage); }
    }
    pub fn handle(&self) -> c_int { self.handle }
}
...
impl Texture {
    pub fn new() -> Result<Self, i32> {
        let mut h: c_int = 0;
        unsafe { glGenTextures(1, &mut h); }
        if h == 0 { Err(0x9994) } else { Ok(Self { handle: h }) }
    }
...
pub struct BatchRenderer<const MAX_VERTICES: usize, const MAX_INDICES: usize> {
    ...
    pub fn draw_quad(&mut self, rect: Rect, uv: Rect, color: Color) {
        ...
        self.vertices[base + 1] = Vertex { pos: Vec2::new(rect.max.x, rect.min.y), uv: Vec2::new(uv.max.x, uv.min.y), color: c };
        self.vertices[base + 2] = Vertex { pos: Vec2::new(rect.max.x, rect.max.y), uv: Vec2::new(uv.max.x, uv.max.y), color: c };
        self.vertices[base + 3] = Vertex { pos: Vec2::new(rect.min.x, rect.max.y), uv: Vec2::new(uv.min.x, uv.max.y), color: c };
        self.vertex_count += 4;
        self.index_count += 6;
    }
...
mod tests {
    ...
    fn test_egl_init() {
        ...
        assert_eq!(maj, 1); assert_eq!(min, 4);
    }
    #[test]
    ...
    fn test_program_link() {
        ...
        p.attach_shader(&vs); p.attach_shader(&fs);
        assert!(p.link().is_ok());
    }
    #[test]
